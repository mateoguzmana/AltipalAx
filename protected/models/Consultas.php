...
class Consultas extends CActiveRecord {
    ...
    public function getSegmentacionIdRespuesta($idRespuesta) {
        $sql = "SELECT IdCodigoSegmentacion FROM `respuestasencuesta` WHERE Id='$idRespuesta'";
    ...
    public function getMenuModulos($perfil) {
        $sql = "SELECT * FROM `configuracionperfil` c INNER JOIN listamenu m ON c.`IdListaMenu`=m.`IdListaMenu` WHERE `IdPerfil`='$perfil' GROUP BY m.`IdListaMenu`";
        return Yii::app()->db->createCommand($sql)->queryAll();
    }
    ...
    public function getCodigoAsesor($nombre) {
        $sql = "SELECT 
                ...
                INNER JOIN zonaventas z ON z.`CodAsesor`=a.`CodAsesor`
                WHERE 
                `Nombre` like '%$nombre%'";
        $connection = Multiple::getConexionZonaVentas();
        return $connection->createCommand($sql)->queryAll();
    ...
    public function getUnidadesConversionArticulo($articulo, $codigoUnidadMedidaProducto, $codigoUnidadMedidaAcuerdo) {
        $sql = "SELECT * 
                FROM `unidadesdeconversion` u
                INNER JOIN operacionesunidades o ON u.CodigoDesdeUnidad=o.CodigoDesde AND u.CodigoHastaUnidad=o.CodigoHasta
                WHERE `CodigoDesdeUnidad`='$codigoUnidadMedidaProducto' AND `CodigoHastaUnidad`='$codigoUnidadMedidaAcuerdo' AND `CodigoArticulo`='$articulo'";
        $connection = Multiple::getConexionZonaVentas();
        return $connection->createCommand($sql)->queryRow();
    }
    ...
    public function getUnidadesConversionArticuloACDM($articulo, $codigoUnidadMedidaProducto, $codigoUnidadMedidaAcuerdo) {
        $sql = "SELECT * 
                ...
                INNER JOIN operacionesunidades o ON u.CodigoDesdeUnidad=o.CodigoDesde AND u.CodigoHastaUnidad=o.CodigoHasta
                WHERE `CodigoDesdeUnidad`='$codigoUnidadMedidaAcuerdo' AND `CodigoHastaUnidad`='$codigoUnidadMedidaProducto' AND `CodigoArticulo`='$articulo'";
        $connection = Multiple::getConexionZonaVentas();
        return $connection->createCommand($sql)->queryRow();
    }
    ...
    public function getUnidadesConversion($codigoArticulo, $desde, $hasta) {
        $sql = "SELECT * 
                ...
                `unidadesdeconversion` u
                INNER JOIN operacionesunidades o ON u.CodigoDesdeUnidad=o.CodigoDesde AND u.CodigoHastaUnidad=o.CodigoHasta
                WHERE `CodigoDesdeUnidad`='$desde' AND `CodigoHastaUnidad`='$hasta' AND `CodigoArticulo`='$codigoArticulo'";
        $connection = Multiple::getConexionZonaVentas();
        return $connection->createCommand($sql)->queryRow();
    }
    ...
    public function actualizarSaldoInventarioPreventa($idSaldo, $cantidadRestar) {
        ...
        $connection = Multiple::getConexionZonaVentas();
        $dataReader = $connection->createCommand($sql)->queryRow();
    ...
    public function getTipoPago($zonaVentas) {
        $sql = "SELECT `AplicaContado`
                FROM `gruposventas` g
                INNER JOIN zonaventas z ON z.`CodigoGrupoVentas`=g.`CodigoGrupoVentas`	
                WHERE 
                z.CodZonaVentas='$zonaVentas'";
        $connection = Multiple::getConexionZonaVentas();
        $command = $connection->createCommand($sql);
    ...
    public function getValorMinimo($cuentaCliente) {
        $sql = "SELECT 
            ce.ValorMinimo 
            FROM `cliente` c
            INNER JOIN cadenaempresa ce ON c.CodigoCadenaEmpresa=ce.CodigoCadenaEmpresa
            WHERE `CuentaCliente`='$cuentaCliente'";
        $connection = Multiple::getConexionZonaVentas();
        $command = $connection->createCommand($sql);
        $dataReader = $command->queryRow();
        return $dataReader;
    ...
    public function InsertPermisoPaginaWeb($agencia, $zona, $fechaini, $fechafin, $observacion) {
        try {
            ...
            Yii::app()->db->createCommand($sql)->query();
            $sql2 = "SELECT MAX(Id) FROM `permisospaginaweb`";
            $id = Yii::app()->db->createCommand($sql2)->queryScalar();
            $sql3 = "INSERT INTO `logpermisosweb`(`Cedula`, `Fecha`, `Idpermisos`) VALUES (" . Yii::app()->user->_cedula . ",CURDATE(),$id)";
            return Yii::app()->db->createCommand($sql3)->query();
        } catch (Exception $ex) {
            echo $ex->getMessage('Error');
            return false;
        }
    ...
    public function getPendientesPorFacturarFilter($zonaVentas, $textFilter) {
        try {
            $sql = "SELECT * FROM `pendientesporfacturar` WHERE SalesZona='$zonaVentas' AND (AccountNum LIKE '%$textFilter%' OR CustName LIKE '%$textFilter%' OR ReasonPendingInovice LIKE '%$textFilter%')";
            $connection = Multiple::getConexionZonaVentas();
            return $connection->createCommand($sql)->queryAll();
        } catch (Exception $ex) {
            echo $ex->getMessage('Error');
    ...
    public function getFacturasSaldoCliente($Nit) {
        try {
            $sql = "SELECT * FROM `facturasaldo` fs
                    INNER JOIN cliente AS cli ON fs.CuentaCliente=cli.CuentaCliente
                    WHERE cli.Identificacion='$Nit'
    ...
    public function getRecibosCajaFacturaNumeroFactura($NumeroFactura) {
        try {
            $sql = "SELECT rf.*,rc.* FROM `reciboscajafacturas` AS rf
                    INNER JOIN `reciboscheque` AS rc ON rf.Id=rc.IdReciboCajaFacturas
    ...
    public function getConteoClienteNuevoPedido($zonaVentas, $cuentaCliente) {
        $sql = "SELECT COUNT(*) AS ConteoCliente FROM `clientenuevo` WHERE `CodZonaVentas`='$zonaVentas' AND `ArchivoXml`='$cuentaCliente' AND `Estado`='1' AND `FechaRegistro`=CURDATE();";
        $connection = Multiple::getConexionZonaVentas();
        return $connection->createCommand($sql)->queryRow();
    }

    public function getClienteNuevoPedido($idPedido) {
        $sql = "SELECT p.IdPedido,p.CodZonaVentas,p.CuentaCliente,p.CodGrupoVenta,p.`ValorPedido`,jc.NombreEmpleado FROM `pedidos` AS p INNER JOIN jerarquiacomercial AS jc ON p.CodZonaVentas=jc.CodigoZonaVentas WHERE p.IdPedido='$idPedido' GROUP BY p.IdPedido;";
        $connection = Multiple::getConexionZonaVentas();
        return $connection->createCommand($sql)->queryRow();
    }

    public function getValorPedidoMaximo() {
        $sql = "SELECT `Valor` FROM `pedidomaximoclientenuevo` LIMIT 1;";
        return Yii::app()->db->createCommand($sql)->queryRow();
    }

    public function getCorreosPedidoClienteNuevo($agencia) {
        $sql = "SELECT a.Id,a.`Nombre`,a.`Apellidos`,a.`CorreoElectronico`,a.`CodAgencia`,a.`Estado` FROM `correospedidosclientenuevo` AS a WHERE a.`Estado`='1' AND a.`CodAgencia`='$agencia' GROUP BY a.`Id`;";
        return Yii::app()->db->createCommand($sql)->queryAll();
    }

    public function getFormasPagoConsigEfectivo($agencia) {
        try {
            $sqlFormaPago = "SELECT CodigoFormadePago,Descripcion FROM `formaspago` WHERE EstadoConsignacion='1';";
            return Multiple::consultaAgencia($agencia, $sqlFormaPago);
        } catch (Exception $ex) {
            echo $ex->getMessage('Error');
            return false;
        }
    ...
    public function getFormasPagoDescription($agencia, $codigoFormaPago) {
        try {
            $sqlFormaPago = "SELECT Descripcion FROM `formaspago` WHERE EstadoConsignacion='1' AND CodigoFormadePago='$codigoFormaPago';";
            return Multiple::consultaAgenciaRow($agencia, $sqlFormaPago);
        } catch (Exception $ex) {
            echo $ex->getMessage('Error');
            return false;
        }
