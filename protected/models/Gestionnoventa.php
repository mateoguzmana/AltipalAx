...
class Gestionnoventa extends CActiveRecord {
    ...
    public function ContarNoventas($cuentacliente) {
        $sql = "SELECT COUNT(CuentaCliente) as noventas FROM `noventas` WHERE `CuentaCliente`='$cuentacliente' AND `FechaNoVenta`=CURDATE()";
    ...
    public function ContarPedidos($cuentacliente) {
        $sql = "SELECT COUNT(CuentaCliente) as pedidos FROM `pedidos` WHERE CuentaCliente='$cuentacliente' AND FechaPedido=CURDATE()";
        $connection = Multiple::getConexionZonaVentas();
        return $connection->createCommand($sql)->queryRow();
    }
