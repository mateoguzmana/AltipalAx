 ...
 */
class DevolucionesModel extends AgenciaActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            array('CodAsesor, CodZonaVentas, CuentaCliente, CodigoMotivoDevolucion, CuentaProveedor, CodigoSitio, TotalDevolucion, ValorDevolucion, FechaDevolucion, HoraFinal, IdDevolucionMaquina, IdentificadorEnvio,ExtraRuta, Ruta', 'required'),
            array('TotalDevolucion, Estado, Web, Autoriza, ExtraRuta, Ruta', 'numerical', 'integerOnly' => true),
     ...
     * @return DevolucionesModel the static model class
     */
   ...
   /* public function getDescripcionDevoluciones($id) {
    ...
    public function getDevoluciones($zonaVentas){
        
        $connection = Multiple::getConexionZonaVentas();
        $sql = "SELECT * FROM `devoluciones` where CodZonaVentas = '$zonaVentas' AND Estado = '0'";
        $command = $connection->createCommand($sql);
        $dataReader = $command->queryAll();
        return $dataReader; 
     ...
     public function UpdateDevolucionTerminarRuta($zona,$agencia){
        
        $consulta = new Multiple();
        $sql = "UPDATE `devoluciones` SET IdentificadorEnvio='1' WHERE CodZonaVentas = '$zona' AND FechaDevolucion=CURDATE() AND IdentificadorEnvio='0'";
