...
class ServiceAltipal extends CActiveRecord {
    ...
    public function insertDatosGlobal($cadena) {
        try {
            return Yii::app()->db->createCommand($cadena)->query();
        } catch (Exception $ex) {
            return FALSE;
        }
    }
    ...
    public function insertProcesosIndividuales($cadena, $status) {
        try {
            $sql = "INSERT INTO `logprocesosindividuales`(`Proceso`, `Status`, `Fecha`, `Hora`) VALUES ('$cadena','$status',CURDATE(),CURTIME())";
            return Yii::app()->db->createCommand($sql)->queryAll();
        } catch (Exception $ex) {
            return FALSE;
        }
...
if ($agencia == '001') {
    ...
    public function getDetallePedidoPreventaService($idPedido, $agencia) {
        try {
            $sql = "SELECT DISTINCT * FROM  descripcionpedido AS descrpedido left join proveedoresestrategicosenvioax as proveeax on descrpedido.CuentaProveedor=proveeax.CuentaProveedor WHERE IdPedido = '$idPedido' ORDER BY IF(proveeax.Item IS NULL,1,0),proveeax.Item,proveeax.CuentaProveedor";
            if ($agencia == '001') {
                return Yii::app()->Apartado->createCommand($sql)->queryAll();
            }
            if ($agencia == '002') {
    ...
    public function getReciboCheque($IdReciboCajaFacturas, $agencia) {
        try {
            $sql = "SELECT *,'002' FROM `reciboscheque` as cheque join `reciboscajafacturas` as reciCaja on reciCaja.Id=cheque.IdReciboCajaFacturas WHERE IdReciboCajaFacturas = '$IdReciboCajaFacturas'";
            if ($agencia == '001') {
                return Yii::app()->Apartado->createCommand($sql)->queryAll();
            }
            if ($agencia == '002') {
    ...
    public function insertDatos001($agencia) {
        try {
            $sql = "SELECT  DISTINCT(`CodZonaVentas`),`CodAgencia`,`NombreAgencia` FROM `zonaventasglobales` WHERE `CodAgencia`='" . $agencia . "' ORDER BY CodAgencia ASC";
            return Yii::app()->db->createCommand($sql)->queryAll();
        } catch (Exception $ex) {
            return FALSE;
        }
    ...
    public function getGrupoImpuestos($tipoDocumento, $barrio, $agencia) {
        try {
            ...
            if ($agencia == '001') {
                $dataReader = Yii::app()->Apartado->createCommand($sql)->queryAll();
            }
            if ($agencia == '002') {
                $dataReader = Yii::app()->Bogota->createCommand($sql)->queryAll();
            }
            if ($agencia == '003') {
                $dataReader = Yii::app()->Cali->createCommand($sql)->queryAll();
            }
            if ($agencia == '004') {
                $dataReader = Yii::app()->Duitama->createCommand($sql)->queryAll();
            }
            if ($agencia == '005') {
                $dataReader = Yii::app()->Ibague->createCommand($sql)->queryAll();
            }
            if ($agencia == '006') {
                $dataReader = Yii::app()->Medellin->createCommand($sql)->queryAll();
            }
            if ($agencia == '007') {
                $dataReader = Yii::app()->Monteria->createCommand($sql)->queryAll();
            }
            if ($agencia == '008') {
                $dataReader = Yii::app()->Pasto->createCommand($sql)->queryAll();
            }
            if ($agencia == '009') {
                $dataReader = Yii::app()->Pereira->createCommand($sql)->queryAll();
            }
            if ($agencia == '010') {
                $dataReader = Yii::app()->Popayan->createCommand($sql)->queryAll();
            }
            if ($agencia == '011') {
                $dataReader = Yii::app()->Villavicencio->createCommand($sql)->queryAll();
            }
            $codigoGrupoImpuesto = "";
    ...
    public function getMensajeCorreo() {
        ...
        try {
            $sql = "SELECT * FROM  `erroresactualizacion` WHERE Fecha >=  '$fechaConsulta' AND MensajeActivity NOT IN ('FINALIZA PROCESO ACTUALIZACIÓN DE INFORMACIÓN',  'INICIO PROCESO ACTUALIZACIÓN DE INFORMACIÓN',  'Error Proceso Clientes Nuevos')";
            return Yii::app()->db->createCommand($sql)->queryAll();
        } catch (Exception $ex) {
            return FALSE;
        }
