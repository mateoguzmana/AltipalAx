...
class Preventa extends AgenciaActiveRecord {
    ...
    public function cargarPortafolio() {
        try {
            ...
            $this->dataReader = $connection->createCommand($sql)->queryAll();
            return true;
        } catch (Exception $exc) {
...
";
    ...
    public function cargarUbicacionZona($zonaVentas) {
        ...
        try {
            ...
            $sql = "SELECT `CodigoUbicacion` FROM `zonaventaalmacen` WHERE `CodZonaVentas`='$zonaVentas' AND Autoventa = 'verdadero'";
