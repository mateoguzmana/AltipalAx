...
SELECT  'Sin información' as IdDocumento,
    ...
    public function getAdministradoresDirectorComercialUsuario($usuario) {
        try {
            $sql = "SELECT * FROM `configuracionadministrador` confi INNER JOIN administrador admin ON confi.IdAdministrador=admin.Id where admin.IdPerfil='12' AND confi.IdAdministrador = '$usuario'";
            return Yii::app()->db->createCommand($sql)->queryAll();
        } catch (Exception $ex) {
            echo $ex->getMessage('Error');
            return false;
    ...
    public function getNotasCreditoDirectorComercialTransmitidas($cedula, $usuario, $fechaIni, $fechaFin) {
        try {
            ...
            $this->ConceptosNotasCredito = $consulta->getConcepto($cedula);
            $Conceptos = "(";
            ...
            $result = $consulta->multiConsultaQuery($sql);
            $nC = 0;
            ...
            $sqlAdmin = "SELECT 
                ...
                FROM `administrador` 
                WHERE `Cedula`='$cedula'";
            $command = $connection->createCommand($sqlAdmin);
            $Nombre = $command->queryRow();
    ...
    public function getGrupoVentas() {
        try {
            $sql = "SELECT distinct(CodigoGrupoVentas),NombreGrupoVentas
            FROM `gruposventas`";
            return Yii::app()->db->createCommand($sql)->queryAll();
        } catch (Exception $ex) {
            echo $ex->getMessage('Error');
    ...
    public function getNombreUsuario($username) {
        $consulta = new Multiple();
        return $consulta->getNombreUsuario($username);
    }
