...
class ServiceAltipal extends CActiveRecord {
    ...
    public function getVersionActualZona($cadena,$agencia) 
	{
        try 
		{
            $sql = $cadena;
    ...
    public function getMensajeCorreo() {
        ...
        try {
            $connection = Yii::app()->db;            
			$sql = "SELECT * FROM  `erroresactualizacion` WHERE ((Fecha='$fechaConsulta' AND Hora>='23:00:00') or (Fecha=curdate())) AND MensajeActivity NOT IN ('Error Proceso Clientes Nuevos') AND ServicioSRF NOT IN ('Servicio Activity','Facturas Transacciones','Pendientes por Facturar','Presupuesto de Ventas')";			
            $command = $connection->createCommand($sql);
            $dataReader = $command->queryAll();
            return $dataReader;
        } catch (Exception $ex) {
	...
	public function getMensajeCorreoResumenProcesoIndividual($identificadorproceso) 
	{
		...
		{
            $connection = Yii::app()->db;
            //$sql = "SELECT * FROM  `logprocesosindividuales` WHERE ((Fecha='$fechaConsulta' AND `Hora`>='01:00:00') OR Fecha = CURDATE( )) AND `IdentificadorEstado`='1' AND `IdentificadorProceso`='".$identificadorproceso."' ORDER BY `Id`";
